...
class AdaptiveSSDPModule(nn.Module):
    """
    - epoch < warmup_epochs: Only record synchrony S and loss; no weight update, return Δw = 0.
    - Upon the first transition past warm-up, automatically fit gate_k, μ_S, and σ_S.
    - Afterwards, scale Δw by gate_b = 1 + k (S − μ) / σ.
    """
    ...
    def _fit_gate(self):
        ...
        self.gate_k = - (S_n * L_n).mean().detach()   
